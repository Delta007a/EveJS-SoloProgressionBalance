...
Use **EveJS Launcher v1.0.65 → Mods → Add ZIP**, select the release ZIP, enable the mod and restart the Game server. The release ZIP starts disabled. In **Configure**, turn on Solo scaling and set the four existing multipliers: mining module yield, mining drone yield, mining hold, and logistics cargo. Verbose logging is an advanced field. The defaults are 3×, 3×, 3× and 2×, but scaling remains off until enabled. A manual Native server can preload `mods/soloProgressionBalance/loader.js` with Node `--require` before EveJS modules load.
...
Final 0.12.9 certification: 44/44 targeted tests, five clean-final port checks and four-loader composition passed. The operational mining-drone attribute merge retains upstream duration and related attributes. Final live gameplay remains to be checked.
